    ...
    >
      <View style={styles.header}>
        <View>
          ...
          <Text style={styles.userName}>
            {user?.first_name ? `${user.first_name} ${user.last_name || ''}` : (user?.role === 'admin' ? 'Administrator' : 'CHW')}
          </Text>
        </View>
      ...
      <View style={styles.syncStatus}>
        ...
        <Text style={styles.syncText}>
          {stats.offline ? 'Operating Offline' : (lastSync ? `Last sync: ${new Date(lastSync).toLocaleString()}` : 'Not synced')}
        </Text>
      </View>
      ...
      <View style={styles.quickActions}>
        {user?.role === 'admin' ? (
          <>
            ...
            <TouchableOpacity style={styles.actionItem} onPress={() => navigation.navigate('SystemPerformance')}>
              <Icon name="analytics" size={24} color="#4CAF50" />
              <Text style={styles.actionText}>System Performance</Text>
              <Icon name="chevron-right" size={24} color="#ccc" />
            </TouchableOpacity>

            <TouchableOpacity style={styles.actionItem} onPress={() => navigation.navigate('ScalingConfig')}>
          ...
          <>
            ...
            <TouchableOpacity style={styles.actionItem} onPress={() => navigation.navigate('AddPatient')}>
              <Icon name="person-add" size={24} color="#2196F3" />
              <Text style={styles.actionText}>Register Patient</Text>
              <Icon name="chevron-right" size={24} color="#ccc" />
            </TouchableOpacity>

            <TouchableOpacity style={styles.actionItem} onPress={() => navigation.navigate('NewVisit')}>
              <Icon name="add-circle" size={24} color="#4CAF50" />
              <Text style={styles.actionText}>New Visit</Text>
              <Icon name="chevron-right" size={24} color="#ccc" />
            </TouchableOpacity>

            <TouchableOpacity style={styles.actionItem} onPress={() => navigation.navigate('Households')}>
...
const styles = StyleSheet.create({
  ...
  syncText: { marginLeft: 8, color: '#666', fontSize: 12 },
  statsGrid: {
    flexDirection: 'row',
    flexWrap: 'wrap',
    padding: 10,
  ...
  statTitle: { fontSize: 12, color: '#666', marginTop: 5 },
